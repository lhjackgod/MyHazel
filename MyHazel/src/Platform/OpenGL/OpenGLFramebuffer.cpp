...
namespace Hazel {
	...
	namespace Utils
	{
		...
		static void AttachDepthTexture(uint32_t id, int samples, GLenum format, GLenum attachmentType, uint32_t width, uint32_t height)
		{
			...
			else
			{
				...
				//glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format, GL_UNSIGNED_BYTE, nullptr); // error
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		if (m_ColorAttachmentSpecifications.size())
		{
			...
			for (size_t i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				case FrambufferTextureFormat::RGBA8:
					...
					break;
				case FrambufferTextureFormat::RED_INTEHER:
					Utils::AttachColorTexture(m_ColorAttachments[i], m_Specification.Samples,
		...
		if (m_DepthAttachmentSpecification.TextureFormat != FrambufferTextureFormat::None)
		{
